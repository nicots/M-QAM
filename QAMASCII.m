function [signal,I,Q,mod,mappedSymbols,signalDemodulated,wiadomosc,bitErrorRate] = QAMASCII( constelationSize, message, signalToNoiseRatio )


numberOfBits = message;
M = constelationSize;
k = log2(M);
SNR = signalToNoiseRatio;
%Generowanie wiadomosci
generatedData = word2bin(numberOfBits);
generatedData = num2str(generatedData) - '0';
...
bitErrorRate = zeros(1,length(SNR)); %prealokacja wektora
    ...
    for p = 100:100:length(demod)
        value = demod(p); % tutaj przekazuje kolejne wartoœci np -1+3*1i
        index = find(mod == value,1); % wyszukuje odpowiedni indeks dla tej wiadomosci w mapie konstelacji
        bin = dec2bin(index-1,log2(M));
        msgdem = [msgdem bin];
    end
    wiadomosc = bin2words(msgdem);
    msgdem = num2str(msgdem) - '0'; % zamieniam wartoœc indeksu na liczbe binarna odpowiadaj¹c¹ danemu punktowi
