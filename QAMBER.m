...
% Wartoœci zwracane:
% bitErrorRate - wektor z wartosci¹ BER dla poszczególnych poziomów SNR
...
M = constelationSize;
k = log2(M); %iloœæ bitów na symbol
...
theoryBer = 2*(((sqrt(M))-1)/(sqrt(M)*k)).*erfc(sqrt(((3*k)/(2*(M-1)))*SNRdB)); %rysuje krzyw¹ teoretyczn¹
